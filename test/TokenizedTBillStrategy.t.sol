...
contract TokenizedTBillStrategyTest is Test {
    ...
    function setUp() public {
        ...
        tBillStrategy = new TokenizedTBillStrategy(
            "T-Bill Strategy", address(usdc), address(tBillToken), tBillProtocol, feeRecipient
        );
    ...
    function test_GetTotalValue() public {
        ...
        assertGt(
            valueAfterTimePass, DEPOSIT_AMOUNT, "Total value should be greater than deposit amount due to interest"
        );
    }
    ...
    function test_HarvestYield() public {
        ...
        assertApproxEqRel(
            usdc.balanceOf(owner), initialOwnerBalance + expectedNetYield, 0.01e18, "Owner should receive net yield"
        );
    ...
    function test_ReentrancyProtection() public view {
        ...
        assertTrue(
            contains(bytecode, abi.encodePacked(depositSelector)), "Deposit function should have nonReentrant modifier"
        );
