...
contract YieldStrategiesTest is Test {
    ...
    function setUp() public {
        ...
        lendingStrategy = new StablecoinLendingStrategy(
            "Stablecoin Lending", address(usdc), lendingProtocol, address(aUsdc), feeRecipient
        );

        tBillStrategy = new TokenizedTBillStrategy(
            "Tokenized T-Bill", address(usdc), address(tBillToken), tBillProtocol, feeRecipient
        );
        ...
        vm.mockCall(tBillProtocol, abi.encodeWithSignature("deposit(uint256)", 0), abi.encode(0));

        vm.mockCall(stakingProtocol, abi.encodeWithSignature("stake(uint256)", 0), abi.encode(0));
        ...
        vm.mockCall(
            stakingProtocol, abi.encodeWithSignature("unstake(uint256)", DEPOSIT_AMOUNT), abi.encode(DEPOSIT_AMOUNT)
        );

        // Add wildcard mocks for any amount
        vm.mockCall(
            stakingProtocol, abi.encodeWithSignature("getBaseAssetValue(uint256)", uint256(0)), abi.encode(uint256(0))
        );
        ...
        vm.mockCall(stakingProtocol, abi.encodeWithSignature("unstake(uint256)", uint256(0)), abi.encode(uint256(0)));
    }
    ...
    function testStakingStrategyDeposit() public {
        ...
        vm.mockCall(
            stakingProtocol, abi.encodeWithSignature("stake(uint256)", DEPOSIT_AMOUNT), abi.encode(DEPOSIT_AMOUNT)
        );
    ...
    function testStakingStrategyWithdraw() public {
        ...
        vm.mockCall(stakingProtocol, abi.encodeWithSelector(getBaseAssetValueSelector), abi.encode(DEPOSIT_AMOUNT));
        ...
        vm.mockCall(stakingProtocol, abi.encodeWithSelector(getStakingTokensSelector), abi.encode(DEPOSIT_AMOUNT));
        ...
        vm.mockCall(stakingProtocol, abi.encodeWithSelector(unstakeSelector), abi.encode(DEPOSIT_AMOUNT));
    ...
    function testHarvestYield() public {
        ...
        assertEq(
            usdc.balanceOf(feeRecipient), initialFeeRecipientBalance + expectedFee, "Fee recipient should receive fee"
        );
