...
contract IndexFundVaultV2Test is Test {
    ...
    function setUp() public {
        ...
        // Deploy the vault
        vault = new IndexFundVaultV2(IERC20(address(mockUSDC)), mockFeeManager, mockPriceOracle, mockDEX);
    ...
    function test_Constructor_InvalidParams() public {
        ...
        new IndexFundVaultV2(IERC20(address(mockUSDC)), mockFeeManager, IPriceOracle(address(0)), mockDEX);

        // Test with zero address for DEX
        vm.expectRevert(CommonErrors.ZeroAddress.selector);
        new IndexFundVaultV2(IERC20(address(mockUSDC)), mockFeeManager, mockPriceOracle, IDEX(address(0)));
    }
    ...
    function test_Rebalance_ComplexWeightChanges() public {
        ...
        vm.mockCall(address(wrapper1), abi.encodeWithSelector(IAssetWrapper.allocateCapital.selector), abi.encode(true));

        vm.mockCall(address(wrapper2), abi.encodeWithSelector(IAssetWrapper.allocateCapital.selector), abi.encode(true));

        vm.mockCall(
            address(yieldWrapper), abi.encodeWithSelector(IAssetWrapper.allocateCapital.selector), abi.encode(true)
        );
        ...
        vm.mockCall(
            address(wrapper1), abi.encodeWithSelector(IAssetWrapper.getValueInBaseAsset.selector), abi.encode(value1)
        );

        vm.mockCall(
            address(wrapper2), abi.encodeWithSelector(IAssetWrapper.getValueInBaseAsset.selector), abi.encode(value2)
        );
        ...
        vm.mockCall(
            address(vault), abi.encodeWithSelector(IndexFundVaultV2.totalAssets.selector), abi.encode(totalValue)
        );
    ...
    function test_Rebalance_FailingAllocate() public {
        ...
        vm.mockCall(
            address(failingWrapper), abi.encodeWithSelector(IAssetWrapper.allocateCapital.selector), abi.encode(false)
        );
    ...
    function test_Rebalance_FailingWithdraw() public {
        ...
        vm.mockCall(
            address(failingWrapper), abi.encodeWithSelector(IAssetWrapper.withdrawCapital.selector), abi.encode(0)
        );
    ...
    function test_HarvestYield_FailingHarvest() public {
        ...
        vm.mockCall(address(failingWrapper), abi.encodeWithSelector(IAssetWrapper.harvestYield.selector), abi.encode(0));
