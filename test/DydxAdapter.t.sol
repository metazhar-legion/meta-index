    ...
    function closePosition(ClosePositionArgs calldata args) external override returns (int256 pnl) {
        ...
        if (position.size > 0) {
            ...
            if (currentPrice > position.entryPrice) {
                // Profit scenario
                pnl = int256(
                    position.collateral * (currentPrice - position.entryPrice) * uint256(position.size)
                        / position.entryPrice
                );
            } else {
                // Loss scenario
                pnl = -int256(
                    position.collateral * (position.entryPrice - currentPrice) * uint256(position.size)
                        / position.entryPrice
                );
            }
        } else {
        ...
        uint256 amountToReturn = pnl > 0 ? position.collateral + uint256(pnl) : position.collateral - uint256(-pnl);
...
contract DydxAdapterTest is Test {
    ...
    event PositionOpened(
        bytes32 indexed positionId, bytes32 indexed marketId, int256 size, uint256 leverage, uint256 collateral
    );
