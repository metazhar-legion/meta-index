...
contract IndexFundVaultV2ComprehensiveTest is Test {
    // Contracts
    ...
    function test_Rebalance_ReentrancyProtection() public {
        ...
        assertEq(maliciousWrapper.getValueInBaseAsset(), DEPOSIT_AMOUNT);
        assertEq(mockUSDC.balanceOf(address(vault)), 0);
    }
    ...
    function test_Withdraw_ReentrancyProtection() public {
        ...
        // Verify that the withdraw completed successfully despite attack attempt
        assertEq(maliciousWrapper.getValueInBaseAsset(), DEPOSIT_AMOUNT / 2);
    }
